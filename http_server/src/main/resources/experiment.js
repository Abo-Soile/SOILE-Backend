...
router.get('/experiment', function (request) {
  experimentDAO.list(
    { users: request.session.currentUser.username },
    function (r) {
  ...
  function renderExp(exp, admin) {
    ...
    if (admin) {
      experimentDAO.countParticipants(id, function (count) {
        ...
        templateManager.render_template(
          'experimentAdmin',
          { exp: experiment, hideLogin: hidelogin },
          request
        );
      });
    } else {
      ...
      templateManager.render_template(
        'experiment',
        { exp: experiment, hideLogin: hidelogin, blockUa: blockUa },
        request
      );
    }
  }
  ...
  dataDAO.get(
    ...
    function (userdata) {
      ...
      if (userdata.position != phaseNo) {
        console.log(
          'Wrong position, redirecting to phase ' + userdata.position
        );
        if (userdata.position == 0) {
          return request.redirect(expUrl);
        }
        ...
        experimentDAO.get(expID, function (exp) {
          ...
          if (phase === undefined) {
            ...
            if (exp.exitButton && !(exp.exitPhase <= phaseNo)) {
            ...
            if (phase.type === 'video') {
              ...
              context.testConfig = {};
              //setting loading text and title
              var loadingText = phase.loadingText;
              console.log(phase);
              var loadingTextTitle = '';
              if (typeof loadingText !== 'undefined') {
                loadingTextTitle = loadingText.split('\n')[0];
                loadingText = loadingText.split('\n').slice(1).join('\n');
          
                loadingText = loadingText.replace(/(?:\r\n|\r|\n)/g, '<br />');

                context.testConfig.loadingText = loadingText;
                context.testConfig.loadingTextTitle = loadingTextTitle;
              }
              //setting text after video and title
              var textAfterVideo = phase.textAfterVideo;
              var textAfterVideoTitle = '';
              if (typeof textAfterVideo !== 'undefined') {
                textAfterVideoTitle = textAfterVideo.split('\n')[0];
                textAfterVideo = textAfterVideo.split('\n').slice(1).join('\n');
          
                textAfterVideo = textAfterVideo.replace(/(?:\r\n|\r|\n)/g, '<br />');

                context.testConfig.textAfterVideo = textAfterVideo;
                context.testConfig.textAfterVideoTitle = textAfterVideoTitle;
              }
              //setting preview instructions and title
              var previewInstructions = phase.previewInstructions;
              var previewInstructionsTitle = '';
              if (typeof previewInstructions !== 'undefined') {
                previewInstructionsTitle = previewInstructions.split('\n')[0];
                previewInstructions = previewInstructions.split('\n').slice(1).join('\n');
          
                previewInstructions = previewInstructions.replace(/(?:\r\n|\r|\n)/g, '<br />');

                context.testConfig.previewInstructions = previewInstructions;
                context.testConfig.previewInstructionsTitle = previewInstructionsTitle;
              }
              ...
              context.testConfig.fullScreen = phase.fullScreen;
              context.testConfig.recordingText = phase.recordingText;

              context.testConfig.startRecordButton = phase.startRecordButton;
              context.testConfig.stopRecordButton = phase.stopRecordButton;

              context.testConfig.minLoadingTime = phase.minLoadingTime;
    ...
    if (exp.exitButton) {
      dataDAO.get(
        ...
        function (userdata) {
          ...
          const end = exp.exitPhase ? exp.exitPhase : totalPhases;
  ...
  experimentDAO.get(expID, function (exp) {
    ...
    if (exp.mechanicalTurkEnabled) {
      dataDAO.getGeneral(userID, expID, function (userData) {
        context.endmessage = context.endmessage.replace(
          '{turkToken}',
          userData.mechanicalTurkToken
        );
